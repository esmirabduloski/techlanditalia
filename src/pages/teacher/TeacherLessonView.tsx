 ...
 export default function TeacherLessonView() {
   ...
   useEffect(() => {
     if (courseId && lessonNumber && hasAccess) {
       fetchData();
     }
   }, [courseId, lessonNumber, hasAccess]);
   ...
   const fetchData = async () => {
     ...
     try {
       ...
       if (lessonData) {
         ...
         if (count && count > 0) {
           navigate(`/insegnante/corso/${courseId}/lezione/${lessonNumber}/task/1`, { replace: true });
           return;
         }
   ...
   const navigateToLesson = (newLessonNumber: number) => {
     navigate(`/insegnante/corso/${courseId}/lezione/${newLessonNumber}`);
   };
