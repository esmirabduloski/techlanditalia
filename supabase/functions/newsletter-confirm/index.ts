...
function createHtmlResponse(siteUrl: string, success: boolean, message: string): Response {
  const html = `
    ...
    <head>
      ...
      <title>${success ? "Iscrizione Confermata" : "Errore"} - TECHLAND</title>
      <style>
    ...
    <body>
      <div class="container">
        ...
        <h1>${success ? "Perfetto!" : "Ops!"}</h1>
        <p>${message}</p>
        ${success ? `<p>Riceverai presto i nostri aggiornamenti su coding, robotica e tecnologia per bambini.</p>` : ""}
